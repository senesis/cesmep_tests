...
compare_script=$root/compare_results.sh
here=$(pwd)
...
# SCRIPT ARGUMENTS
##########################################
# Name of the comparison to reproduce (supposed to be in current script's dir)
...
#   If relative path -> take it in current dir
if [ ${ref_results:0:1} != "/" ]; then
    ref_results=$root/$ref_results
fi
...
cd $CESMEP;
rm -fr $testcomp; cp -a $root/$reference $testcomp ;
(
    cd $testcomp ;
